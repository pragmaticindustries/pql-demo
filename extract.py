...
class ReferenceFieldType(FieldType):
    ...
    def apply(self,parent_entity:str,state:str,end_result:dict) -> Any:
        raise NotImplementedError
...
class AutoGeneratedOneToOne(ReferenceFieldType):
    ...
    def apply(self,parent_entity:str,state:str,end_result:dict):
        if state == self.config_entity:
            end_change = len(end_result[parent_entity])
            for i in range(self.change_index,end_change):
                end_result[parent_entity][i].update({self.config_entity:end_result[self.config_entity][-1].get(self.relation_id)})
                self.constraints.update({'id':end_result[parent_entity][i].get("id"),self.config_entity:end_result[self.config_entity][-1].get(self.relation_id)})
...
class StateProcessor(object):
    ...
    def __exctract_all_items__(self, entity, state):
        all_items: dict = {k: v.apply(state) for k, v in self.config.get(entity).items() if
                           v.is_generated() and not v.is_foreignkey()}
