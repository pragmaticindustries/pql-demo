...
class AutoGeneratedOneToOne(FieldType):
### Idee einer Speichert hier als dict die Id des anderen also so
## {relation: 4711}
##Also wenn der Stategleich ist und hier eine Relation dannn id speichern
    def __init__(self) -> None:
        self.uuid = 0
...
class AutoGeneratedOneToMany(FieldType):
    ...
    * ToolEquipped
    ...
    config: dict = {
        "Cycle": {
            ...
            "MaterialEquipped": AutoGeneratedOneToOne()
        },
        "ToolEquipped": {
    ...
    class StateProcessor(object):
        ...
        def __init_context__(self,first_element_of_stream):
            for entity in self.config.keys():
                ...
                all_items = self.__exctract_all_items__(entity, first_element_of_stream)
                self.end_result[entity].append(all_items)

            for el in self.end_result.keys():
                try:
                # relation_key = {k for k,v in self.end_result[el].items() if k in self.config.keys()}
                # print(relation_key)
                    dict_e = {k for k,v in self.end_result[el][0].items() if k in self.config.keys()}
                    self.end_result[el][-1].pop(dict_e.pop())
                    self.end_result[el][-1].update({dict_e.pop:self.end_result[dict_e.pop()][-1].get('id')})
                    #print(self.end_result[dict_e.pop()][-1].get('id'))
                  #  print(dict_e)
                except Exception as ex:
                    print(ex)

            # for entity in self.config.keys():
            #     all_items:dict = {k: v.apply(first_element_of_stream) for k, v in self.config.get(entity).items() if v.is_generated()}
            #     #print(all_items)
            #     all_items.update(self.context[entity])
               # all_items.update({"start":sta})
            ...
            for entity in config.keys():
                ...
                if not is_last:
                    if self.context[entity] != current_context:
                        ...
                        ### hier ke drin aber kann sein das relation key leer dann nur set und dann kommt fehler
                        ## hier dann prüfen ob sich in state für config was ändert oder nicht
                        ## dann relation_key apply und dann appenden.

                        ### nächstes Schritt dann prüfen ob  der KEy on to One schon genutzt wirde oder nicht wenn ja fehler wenn nicht alles gut.
                         ## oder andere Idee Methode mache für checken ob und welcher relation key da ist und dann gleich die id geben.
                        relation_key = {k for k,v in all_items.items() if k in self.config.keys()}
                        try:
                            key_extract = relation_key.pop()
                            all_items.pop(key_extract,None)
                            # hier Fehler weil minus 1 beim Array
                            # hier dann nochmal alles durch gehen bis der Relkey Kommt und dann den hier einfügen
                            rel_dict:dict = config.get(key_extract)
                            print(rel_dict)
                            rel_id=self.end_result[key_extract][-1].get('id')
                            all_items.update({key_extract:rel_id})
