    ...
    def apply(self,parent_entity:str,state:str,end_result:dict,key):
        ...
        if state == self.config_entity:
            ...
            for i in range(self.change_index,end_change):
                if not any({'id':end_result[parent_entity][i].get("id"),self.config_entity:end_result[self.config_entity][-1].get(self.relation_id)} in self.constraints):
                    end_result[parent_entity][i].update({F"{key}":end_result[self.config_entity][-1].get(self.relation_id)})
                    self.constraints.append({'id':end_result[parent_entity][i].get("id"),self.config_entity:end_result[self.config_entity][-1].get(self.relation_id)})
                else:
                    raise RuntimeError(f"There is allready an existing Entry for {'id':end_result[parent_entity][i].get('id'),self.config_entity:end_result[self.config_entity][-1].get(self.relation_id)}")
...
class AutoGeneratedOneToMany(ReferenceFieldType):
    ...
    def apply(self,parent_entity:str,state:str,end_result:dict,key):
        ...
        if state == self.config_entity:
            end_change = len(end_result[parent_entity])
            for i in range(self.change_index,end_change):
                if not any(d.get('id', None) == end_result[parent_entity][i].get("id") for d in self.constraints):
                    end_result[parent_entity][i].update({F"{key}":end_result[self.config_entity][-1].get(self.relation_id)})
                    self.constraints.append({'id':end_result[parent_entity][i].get("id"),self.config_entity:end_result[self.config_entity][-1].get(self.relation_id)})
                else:
                    raise RuntimeError(f"There is allready an existing Entry for {end_result[self.config_entity][-1].get(self.relation_id)}")
            self.change_index = end_change
        ...
        if state == self.config_entity:
            ...
            for i in range(self.change_index,end_change):
                if not any(d.get('id', None) == end_result[parent_entity][i].get("id") for d in self.constraints):
                    ...
                    raise RuntimeError(f"There is allready an existing Entry for {end_result[parent_entity][i].get('id')}")
            self.change_index = end_change
...
class StateProcessor(object):
    ...
    def __exctract_all_items__(self, entity, state):
        all_items: dict = {k: v.apply(state) for k, v in self.config.get(entity).items() if not v.is_foreignkey()}
        all_items.update({"start": state.get("timestamp"), "end": state.get("timestamp")})
        return all_items

    def __extract_all_not_autogen_items__(self,entity,state):
        all_items: dict = {k: v.apply(state) for k, v in self.config.get(entity).items() if not v.is_generated() and not v.is_foreignkey()}
        all_items.update({"start": state.get("timestamp"), "end": state.get("timestamp")})
        return all_items
    ...
    def __init_context__(self, first_element_of_stream):
        ...
        for entity in self.config.keys():
            x = {k: v.apply(first_element_of_stream) for k, v in self.config.get(entity).items() if not v.is_foreignkey()}
            x.update({"start":first_element_of_stream.get("timestamp"),"end":first_element_of_stream.get("timestamp")})
            self.timestamp.update({entity:first_element_of_stream.get("timestamp")})
            self.context[entity] =  {k: v.apply(first_element_of_stream) for k, v in self.config.get(entity).items() if
    ...
    def process_state(self, state):
        for entity in self.config.keys():
            current_context = {k: v.apply(state) for k, v in self.config.get(entity).items() if
                               not v.is_generated() and not v.is_foreignkey()}
            self.end_result[entity][-1]["end"] = self.timestamp[entity]
            ...
            self.timestamp[entity] = state.get("timestamp")
        self.process_foreingkeys(entity)


    def process_foreingkeys(self, state):
        for entity in self.config.keys():
            {k:v.apply(entity,state,self.end_result,k) for k, v in self.config.get(entity).items() if v.is_generated() and v.is_foreignkey()}
